...
from doctor.functions.for_print_forms import update_UsersToday
from doctor.functions.helpers import formatting_full_name_mode_full
from nutritionist.models import CustomUser, UsersToday
from doctor.functions.functions import create_user, is_user_look
...
class CustomUserCreateTests(unittest.TestCase):
    ...
    def test_correct_user_save(
        ...
        self.assertEqual(current_user.full_name, self.data["full_name"])
        self.assertEqual(
            current_user.birthdate.strftime("%d.%m.%Y"), self.data["birthdate"]
        )
        self.assertEqual(
            current_user.receipt_date.strftime("%d.%m.%Y"), self.data["receipt_date"]
        )
        self.assertEqual(
            current_user.receipt_time.strftime("%H:%M:%S"), self.data["receipt_time"]
        )

        self.assertEqual(current_user.floor, self.data["floor"])
        self.assertEqual(current_user.department, self.data["department"])
        self.assertEqual(current_user.room_number, self.data["room_number"])
        self.assertEqual(current_user.bed, self.data["bed"])

        self.assertEqual(current_user.type_of_diet, self.data["type_of_diet"])
        self.assertEqual(current_user.comment, self.data["comment"])
        self.assertEqual(current_user.status, self.data["status"])
        ...
        self.assertEqual(current_user.type_pay, self.data["type_pay"])
    ...
    def test_is_user_look(self):
        ...
        self.assertEqual(wrong_response, None)

    def test_formatting_full_name_mode_full(self):
        full_name_row = "Егорова Антонина Степановна"
        response = formatting_full_name_mode_full(full_name_row)

        self.assertEqual(full_name_row, response)

        full_name_row_1 = "егорова антонина степановна"
        response_1 = formatting_full_name_mode_full(full_name_row_1)
        self.assertEqual(full_name_row, response_1)

    def test_update_user_today(self):
        current_user = CustomUser.objects.last()
        current_user_full_name = current_user.full_name
        response = update_UsersToday(current_user)
        self.assertEqual(response, None)

        current_user_today = UsersToday.objects.get(user_id=current_user.id)
        self.assertEqual(current_user_today.full_name, current_user_full_name)
        self.assertEqual(current_user_today.department, current_user.department)
        self.assertEqual(current_user_today.room_number, current_user.department)
        self.assertEqual(current_user_today.floor, current_user.floor)