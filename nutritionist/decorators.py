...
def login_required_accountant(func):
    def wrapper(request, *args, **kwargs):
        if request.user.is_anonymous or not check_group_accountant(request.user):
            messages.info(request, "Для просмотра этой страницы нужно войти в систему как бухгалтер-калькулятор!")
            return redirect("login")
        else:
            return func(request, *args, **kwargs)
    return wrapper
...
def login_required_manager_and_kitchen(func):
    def wrapper(request, *args, **kwargs):
        if request.user.is_anonymous or (not check_group_kitchen(request.user) and not check_group_manager(request.user)):
            messages.info(request, "Для просмотра этой страницы нужно войти в систему как персонал кухни или менджер!")
            return redirect("login")
        else:
            return func(request, *args, **kwargs)
    return wrapper
